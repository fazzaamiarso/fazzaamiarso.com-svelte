<script lang="ts">
	...
	function getScrollAmount(el: HTMLElement | null) {
		return (el?.offsetWidth ?? 0) - window.innerWidth + (el?.getBoundingClientRect().left ?? 0) * 2;
	}
	onMount(() => {
		...
		mm.add('(min-width:1024px)', () => {
			...
			gsap.to(horizontalPanels, {
				ease: 'none',
				x: () => -1 * getScrollAmount(horizontalContainer),
				scrollTrigger: {
					pin: horizontalContainer,
					trigger: '.h-timeline h2',
					scrub: 1,
					start: 'top top',
					end: () => {
						return '+=' + getScrollAmount(horizontalContainer);
					},
...
<div
	class="h-container flex flex-col gap-6 lg:flex-row lg:flex-nowrap ml-auto lg:w-max lg:gap-32 lg:items-center lg:pt-40">
	<div class="h-panel ring-brand-700">
		<p class="text-body-lg md:text-body-xl mb-8 w-full">
		...
		<div class=""><img src={scratch} alt="scratch logo" class="w-60 mx-auto" /></div>
	</div>
	<div class="h-panel ring-brand-700">
	...
	<div class="h-panel ring-brand-700">
		...
		<p class="text-body-lg md:text-body-xl w-full">
			Aaaaand... I dropped it too. Later on, I was amazed by Web development in the end of High School from seeing a
			Youtube Video and unsurprisingly keep going until present.
		</p>
	</div>
</div>
